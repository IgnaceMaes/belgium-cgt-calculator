...
import type { LineChartSeries, LineChartPoint } from '@/components/cgt/line-chart';
...
export default class CgtCalculator extends Component {
  ...
  get holdTax() { return holdTotalTax(this.holdResults); }
  get harvestTax() { return harvestTotalTax(this.harvestResults); }
  get smartTax() { return smartTotalTax(this.smartResults); }
  ...
  get scenarios() {
    return [
      {
        name: 'Hold', key: 'hold', color: HOLD_COLOR,
        final: this.holdFinal, totalTax: this.holdTax,
        grossProfit: this.holdFinal + this.holdTax - this.totalInvested,
        netProfit: this.holdFinal - this.totalInvested,
        borderClass: 'border-indigo-500/30', bgClass: 'border-indigo-500/30 bg-indigo-500/10',
        badgeClass: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30',
      },
      {
        name: 'Full harvest', key: 'harvest', color: HARVEST_COLOR,
        final: this.harvestFinal, totalTax: this.harvestTax,
        grossProfit: this.harvestFinal + this.harvestTax - this.totalInvested,
        netProfit: this.harvestFinal - this.totalInvested,
        borderClass: 'border-pink-500/30', bgClass: 'border-pink-500/30 bg-pink-500/10',
        badgeClass: 'bg-pink-500/20 text-pink-400 border-pink-500/30',
      },
      {
        name: 'Smart harvest', key: 'smart', color: SMART_COLOR,
        final: this.smartFinal, totalTax: this.smartTax,
        grossProfit: this.smartFinal + this.smartTax - this.totalInvested,
        netProfit: this.smartFinal - this.totalInvested,
        borderClass: 'border-emerald-500/30', bgClass: 'border-emerald-500/30 bg-emerald-500/10',
  ...
  get growthSeries(): LineChartSeries[] {
    ...
    const toPoints = (results: YearResult[]): LineChartPoint[] => [
      ...
      ...results.map((r) => ({ x: r.year, y: r.netPortfolioAfterTax, unrealizedGain: r.unrealizedGain })),
    ];
    return [
      { label: 'Hold', color: HOLD_COLOR, points: toPoints(this.holdResults) },
      { label: 'Full harvest', color: HARVEST_COLOR, points: toPoints(this.harvestResults) },
      { label: 'Smart harvest', color: SMART_COLOR, points: toPoints(this.smartResults) },
    ];
  }
