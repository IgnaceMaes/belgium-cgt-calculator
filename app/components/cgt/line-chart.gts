...
export interface LineChartPoint { x: number; y: number; unrealizedGain?: number; }
export interface LineChartSeries { label: string; color: string; points: LineChartPoint[]; }
...
export default class LineChart extends Component<LineChartSignature> {
  ...
  get tooltipY(): number { return this.padT + 8; }
