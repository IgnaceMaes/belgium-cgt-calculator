...
module('Unit | Utils | tax-calc', function () {
  ...
  module('calcTob', function () {
    test('shares: applies 0.12% rate', function (assert) {
      ...
      assert.true(
        Math.abs(result - 120) < 0.01,
        `expected ~120, got ${result}`,
      );
    });
  ...
  module('holdScenario', function () {
    ...
    test('no tax paid until final year', function (assert) {
      ...
      for (let i = 0; i < 4; i++) {
        assert.strictEqual(
          results[i]!.cgtDue,
          0,
          `year ${i + 1} CGT should be 0`,
        );
        assert.strictEqual(
          results[i]!.tobPaid,
          0,
          `year ${i + 1} TOB should be 0`,
        );
      }
    ...
    test('portfolio grows each year', function (assert) {
      ...
      for (let i = 1; i < 5; i++) {
        assert.true(
          results[i]!.portfolioValue > results[i - 1]!.portfolioValue,
        );
      }
    });
  ...
  module('harvestScenario', function () {
    ...
    test('broker opt-in: withholds 10% on full gain', function (assert) {
      ...
      assert.true(
        Math.abs(results[0]!.cgtDue - expectedCgt) < 1,
        'CGT should be ~10% of gain',
      );
    });
    ...
    test('broker opt-in: refund arrives 2 years later', function (assert) {
      ...
      assert.true(
        results[1]!.refundReceived > 0,
        'should receive refund in year 2',
      );
    });

    test('self-report net > broker net (broker loses compounding)', function (assert) {
      const selfReport = harvestScenario(
        200_000,
        150_000,
        0.07,
        10,
      ...
      assert.true(
        selfNet > brokerNet,
        `self-report (${selfNet}) should be > broker (${brokerNet})`,
      );
    });
  ...
  module('smartScenario', function () {
    ...
    test('final year sells everything (no unrealized gain left)', function (assert) {
      ...
      assert.strictEqual(
        last.unrealizedGain,
        0,
        'unrealized gain should be 0 after final exit',
      );
    });
    ...
    test('self-report: realizes some gain each year', function (assert) {
      ...
      for (const r of results) {
        assert.true(
          r.realizedGain >= 0,
          `year ${r.year} should have non-negative realized gain`,
        );
      }
    });
    ...
    test('smart harvest final net >= full harvest final net', function (assert) {
      ...
      const harvestR = harvestScenario(
        200_000,
        150_000,
        0.07,
        10,
      ...
      assert.true(
        smartNet >= harvestNet,
        `smart (${smartNet}) >= harvest (${harvestNet})`,
      );
    });
    ...
    test('broker opt-in: CGT is always 10% of realized gain', function (assert) {
      ...
      for (const r of withGains) {
        assert.true(
          Math.abs(r.cgtDue - r.realizedGain * CGT_RATE) < 1,
  ...
  module('final value helpers', function () {
    test('holdFinalNet returns last year net', function (assert) {
      ...
      assert.strictEqual(
        holdFinalNet(results),
        results[4]!.netPortfolioAfterTax,
      );
    });
  ...
  module('total tax helpers', function () {
    ...
    test('harvestTotalTax: broker opt-in subtracts refunds', function (assert) {
      ...
      const total = harvestTotalTax(results);
      assert.true(
        Math.abs(total - (gross - refunds)) < 0.01,
        'total tax should be gross minus refunds',
      );
    ...
    test('broker opt-in: pending refunds flushed into final year', function (assert) {
      ...
      assert.true(
        totalRefunds > 0,
        'some refunds should have been received or flushed',
      );
    });
  });
  ...
  module('constants', function () {
    test('CGT rate is 10%', function (assert) {
      assert.strictEqual(CGT_RATE, 0.1);
    });
  ...
  module('edge cases', function () {
    test('zero gain → no CGT in any scenario', function (assert) {
      ...
      assert.strictEqual(
        holdTotalTax(hold),
        hold[4]!.tobPaid,
        'hold: only TOB at exit',
      );
      for (const r of harvest)
        assert.strictEqual(r.cgtDue, 0, 'harvest: no CGT');
    ...
    test('negative return → no CGT, portfolio shrinks', function (assert) {
      ...
      for (const r of results) {
        ...
        assert.strictEqual(
          r.realizedGain,
          0,
          `year ${r.year}: no realized gain`,
        );
      }
      assert.true(
        results[2]!.portfolioValue < 100_000,
        'portfolio should shrink',
      );
    });
    ...
    test('gain exactly at exemption boundary: €10,000', function (assert) {
      ...
      assert.strictEqual(
        newCarryForward,
        0,
        'carry-forward resets when fully used',
      );
    });
    ...
    test('gain at €10,001 → tax on €1', function (assert) {
      ...
      assert.true(Math.abs(tax - 0.1) < 0.01, 'tax should be ~€0.10');
    });
  ...
  module('no loss carry-forward (Belgian law)', function () {
    test('harvest: loss year followed by gain year → no loss offset', function (assert) {
      ...
      const results2 = harvestScenario(
        postLossValue,
        postLossValue,
        0.3,
        1,
      ...
      const gain = postLossValue * 1.3 - postLossValue;
      assert.true(gain > 10_000, 'gain should exceed exemption');
      assert.true(
        results2[0]!.cgtDue > 0,
        'CGT should be charged (no loss carry)',
      );
    });
  ...
  module('carry-forward detailed', function () {
    ...
    test('carry-forward never exceeds €5,000 even after 10 years', function (assert) {
      ...
      for (let i = 4; i < 9; i++) {
        assert.strictEqual(
          results[i]!.carryForward,
          5_000,
          `year ${i + 1}: capped at 5K`,
        );
      }
    });
    ...
    test('carry-forward consumed on exit → effective exemption €15K', function (assert) {
      ...
      assert.true(
        last.exemptionUsed > 10_000,
        'should use more than base exemption',
      );
      assert.true(
        last.exemptionUsed <= 15_000,
        'should not exceed max exemption',
      );
    });
  });
  ...
  module('conservation of money', function () {
    test('hold: value_in + growth = net_out + total_tax', function (assert) {
      ...
      assert.true(
        Math.abs(netOut + totalTax - finalValue) < 0.01,
    ...
    test('harvest (self-report): money is conserved each year', function (assert) {
      ...
      for (const r of results) {
        ...
        assert.true(
          Math.abs(r.netPortfolioAfterTax - expectedNet) < 1,
  ...
  module('yearly contributions', function () {
    test('contributions increase both value and basis', function (assert) {
      ...
      assert.true(
        holdFinalNet(withC) > holdFinalNet(without) + 40_000,
    ...
    test('contributions with negative return still add to basis', function (assert) {
      ...
      for (const r of results) {
        assert.strictEqual(
          r.cgtDue,
          0,
          `year ${r.year}: no CGT (contributions > losses)`,
        );
      }
    });
  });
  ...
  module('broker vs self-report detailed', function () {
    test('broker always withholds more upfront than self-report', function (assert) {
      ...
      for (let i = 0; i < 5; i++) {
        assert.true(
          brokerR[i]!.cgtDue >= selfR[i]!.cgtDue,
    ...
    test('self-report uses exemption, broker does not', function (assert) {
      ...
      assert.strictEqual(
        selfR[0]!.cgtDue,
        0,
        'self-report: no CGT (within exemption)',
      );
      assert.true(
        brokerR[0]!.cgtDue > 0,
        'broker: CGT withheld (ignores exemption)',
      );
    });
  ...
  module('smart harvest specifics', function () {
    test('non-final years: realized gain ≤ exemption (self-report)', function (assert) {
      ...
      for (const r of results.slice(0, -1)) {
        assert.true(
          r.realizedGain <= CGT_EXEMPTION + MAX_CARRY_FORWARD + 1,
    ...
    test('final year: sells everything, unrealized gain = 0', function (assert) {
      ...
      assert.true(
        last.realizedGain > 0,
        'final year should have realized gain',
      );
    });
    ...
    test('smart with very small gains → carry-forward accumulates', function (assert) {
      ...
      assert.true(
        results[3]!.carryForward > 0,
        'carry-forward should accumulate',
      );
    });
  });
  ...
  module('portfolio tax', function () {
    ...
    test('hold: portfolio tax reduces final value', function (assert) {
      ...
      assert.true(
        holdFinalNet(withPtax) < holdFinalNet(without),
        'portfolio tax should reduce final net',
      );
      // Each year should have portfolio tax > 0
      for (const r of withPtax) {
        assert.true(
          r.portfolioTax > 0,
          `year ${r.year}: should have portfolio tax`,
        );
      }
    });

    test('harvest: portfolio tax included in total tax', function (assert) {
      const withPtax = harvestScenario(
        1_500_000,
        1_500_000,
        0.07,
        3,
      ...
      assert.true(
        totalTax > totalPtax,
        'total tax should include portfolio tax + CGT + TOB',
      );
    });

    test('smart: portfolio tax included in total tax', function (assert) {
      const withPtax = smartScenario(
        1_500_000,
        1_500_000,
        0.07,
        3,
    ...
    test('below threshold: no portfolio tax even when enabled', function (assert) {
      ...
      for (const r of results) {
        assert.strictEqual(
          r.portfolioTax,
          0,
          `year ${r.year}: no portfolio tax below €1M`,
        );
      }
    });
    ...
    test('portfolio grows past threshold mid-simulation', function (assert) {
      ...
      assert.true(
        lateYears.length > 0,
        'should start paying portfolio tax after crossing 1M',
      );
